...
%
for iter = 1:cfg.max_penalty_iter
    % YOUR CODE HERE
    ...
    % constraints are not satisfied within tolerance
    if any(g(x) > cfg.cnt_tolerance) || any(abs(h(x)) > cfg.cnt_tolerance)
    ...
    for iter = 1:cfg.max_iter
        ...
        while true 
            ...
            cvx_begin quiet
				...
				%YOUR CODE HERE
				minimize('fval+fgrad*(xp-x)+.5*transpose(xp-x)*(fhess*(xp-x)) + fquadlin(xp) + penalty_coeff * ( hinge(gval+gjac*(xp-x)) + abssum(hval+hjac*(xp-x)) )')
                subject to
                A_ineq*xp <= b_ineq;
                A_eq*xp == b_eq;
                abs(xp - x) <= trust_box_size;
